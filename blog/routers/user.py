...
from ..schemas import User as UserSchema, ShowUser as ShowUserSchema, ShowUserSchemaWithoutBlogs as ShowUserSchemaWithoutBlogsSchema
...
@router.post('/user', response_model=ShowUserSchemaWithoutBlogsSchema, tags=["users"])
...
async def show_users(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(UserModel).options(selectinload(UserModel.blogs)))
...
async def show_user(id: int, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(UserModel).where(UserModel.id == id).options(selectinload(UserModel.blogs)))
